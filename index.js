...
const { config, clients, certificates } = require('./settings');                //get settings of 3 property

const issuer = process.env.ISSUER || 'http://localhost:3000';                   //define ISSUER

config.findById = Account.findById;
// console.log(config.findById)
...
provider.initialize({
  // adapter: process.env.MONGODB_URI ? require('./adapters/mongodb') : undefined, //get client's setting
  clients,
